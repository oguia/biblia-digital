...
var appRouter = router({
  ...
  bible: router({
    getVerses: publicProcedure.input(z2.object({
      book: z2.string(),
      chapter: z2.number(),
      version: z2.string().default("nvi")
    })).query(async ({ input }) => {
      try {
        ...
        if (!book) {
          return { verses: [], error: `Livro '${input.book}' n\xE3o encontrado` };
        }
        const chapter = book.chapters[input.chapter.toString()];
        if (!chapter) {
          return { verses: [], error: `Cap\xEDtulo ${input.chapter} n\xE3o encontrado` };
        }
        ...
        return { verses: [], error: "Erro ao buscar vers\xEDculos" };
      }
    }),
