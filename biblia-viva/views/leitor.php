        ...
        <section class="lg:w-2/3">
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm mb-6 flex flex-wrap gap-4 items-center justify-between border border-gray-100">
                <div class="flex items-center gap-2">
                    <select id="livroSelect" class="bg-white border border-gray-200 text-lg font-bold text-secondary rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary outline-none">
                    ...
                    <div class="flex items-center">
                        <a href="?livro=<?= $livroId ?>&cap=<?= max(1, $capitulo - 1) ?>&versao=<?= $versaoId ?>" class="p-2 hover:bg-gray-100 rounded-full transition <?= $capitulo <= 1 ? 'opacity-50 pointer-events-none' : '' ?>">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                        </a>
                        <span class="text-xl font-bold px-2"><?= $capitulo ?></span>
                        <!-- Nota: Idealmente verificar se existe próximo capítulo -->
                        <a href="?livro=<?= $livroId ?>&cap=<?= $capitulo + 1 ?>&versao=<?= $versaoId ?>" class="p-2 hover:bg-gray-100 rounded-full transition">
